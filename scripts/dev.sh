...
# so native dev runs pick up optional integrations.
if [[ -f .env ]]; then
  set -a
...
if ! docker ps --filter "name=^choicehammer_postgres$" --format '{{.Names}}' | grep -q .; then
  ...
  docker start choicehammer_postgres >/dev/null 2>&1 || \
    docker run -d \
      --name choicehammer_postgres \
      -e POSTGRES_USER=choicehammer \
      -e POSTGRES_PASSWORD=choicehammer \
      -e POSTGRES_DB=choicehammer \
      -p 5434:5432 \
      -v choicehammer_pgdata:/var/lib/postgresql/data \
      postgres:16-alpine >/dev/null

  until docker exec choicehammer_postgres pg_isready -U choicehammer >/dev/null 2>&1; do
    sleep 1
  done
fi
